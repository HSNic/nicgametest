...
>    - 偵測方法:同步套用完、commit 前,對這三塊各自 `git diff`(或直接 `grep`)確認客製標記還在;不在的話回頭找上面列的來源 commit,把對應區塊的內容重新套到當次同步後的新檔案上(不能整份還原成舊 commit 的檔案——那樣會把原作者這次真正的新內容也一起蓋掉,只能挑「客製相關的那幾塊」手動補回)。
