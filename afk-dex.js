...
(function () {
  ...
  function cardHTML(h, mult, q) {
    ...
    var dropsHTML = h.drops.length
      ? '<table class="m-dex-drops"><tbody>' + h.drops.map(function (d) {
          var isRelic = !!(DB.items[d[0]] && DB.items[d[0]].relic);
          var pct, pctCls = 'm-dex-pct';
          if (mult < 1 && isRelic) {
            // 🔒 2026-07-18 經典模式：遺物固定鎖死 0.0001%，不吃怪物原始機率/席琳倍率/兔腳倍率
            pct = 0.0001; pctCls += ' c-relic';
          } else {
            ...
            pct = d[2] * effMult; if (pct > 100) pct = 100;
          }
          var tag = d[3] ? ' <span class="m-dex-droptag">' + esc(d[3]) + '</span>' : '';
          return '<tr>' +
            '<td><span class="m-dex-iname' + (isRelic ? ' c-relic' : '') + '" data-id="' + esc(d[0]) + '" title="看詳情">' + hl(d[1], q) + '</span>' + tag + '</td>' +
  ...
  var SPECIAL_BLOCKS = [
    { id: 'dropmult', title: '🔮 掉落倍率：席琳的世界 ×3、瘋狂的席琳世界 ×5、恩賜怪 ×10', keys: ['掉落倍率', '倍率', '席琳的世界', '瘋狂的席琳世界', '經典模式', '恩賜怪', '恩賜', '遺物'], lines: [
      '<b>怪物卡上顯示的是「一般」掉落機率</b>，下列狀態會整體放大／縮小：',
      '<b>席琳的世界</b>：被席琳化的怪掉落機率 <b>×3</b>（<b>瘋狂的席琳世界</b> <b>×5</b>）；其中「恩賜怪」更高，<b>×10</b>',
      '<b>經典模式</b>：舊有的「所有物品掉落 ×1/10」懲罰已於過去版本移除，目前一般掉落跟一般模式相同；<b>唯一例外是「遺物」品質道具，經典模式下固定鎖死 0.0001%</b>（最後更新：2026-07-18）',
      '<b>固定、不受倍率影響的例外</b>：席琳結晶、萬能藥（屬性藥）、黑魔石、銀礦石、進化果實（各條另有標註）；其餘怪卡掉落都會被倍率放大／縮小',
      '上方的「<b>掉落率模式</b>」下拉可把席琳倍率／經典模式的遺物固定值直接套到怪卡的掉落數字'
    ] },
    ...
    { id: 'dragonegg', title: '🐉 幼龍蛋（頑皮／淘氣，四大龍各 10% 掉落）', keys: ['幼龍蛋', '林德拜爾', '頑皮幼龍蛋', '淘氣幼龍蛋', '頑皮龍', '淘氣龍'], lines: [
      '擊敗<b>安塔瑞斯／法利昂／巴拉卡斯／林德拜爾</b>（四大龍）時，「頑皮幼龍蛋」與「淘氣幼龍蛋」各自獨立判定 <b>10%</b> 機率掉落，可重複取得、<b>不受</b>經典模式掉率影響',
      '可存入倉庫，非唯一道具（跟舊版不同，這批已開放收倉庫）',
      '身上持有任意一種幼龍蛋，於<b>任何野外地圖</b>都有極小機率改為刷出隱藏 BOSS 風龍「林德拜爾」（Lv90，場上沒有其他頭目時才出現、同時最多一隻）；蛋全數收進倉庫或賣出後，就不再引來風龍',
      '<b>直接使用</b>該道具（寵物保管未滿時）：「頑皮幼龍蛋」孵出「頑皮龍」、「淘氣幼龍蛋」孵出「淘氣龍」，一出生即最終型態的寵物，不用像其他寵物一樣練到 30 級再進化'
    ] },
    { id: 'pledgedrop', title: '🎁 野外血盟敵人／攻城敵人／假玩家 額外掉落', keys: ['攜帶物', '假玩家', '白目玩家'], lines: [
      '擊殺時額外掉一件物品：從幾乎所有可掉物依稀有度隨機抽（越稀有越難中、常見物權重加倍）',
      '一般血盟野外/攻城敵人機率固定 1%；<b>假玩家/白目玩家 NPC 依對方性向值 3%~10%（越邪惡機率越高）</b>，詳細對照表見小百科「性向與PVP」分頁',
      '抽到裝備一定帶強化：多在 +0～該裝備安定值；超出安定值的機率 +1 為 0.1%、+2 0.01%、+3 0.001%、+4 0.0001%。另 1% 帶「祝福的」',
      '攻城區域內擊殺不會觸發這個掉落'
    ] },
    { id: 'playernpcrelic', title: '🏺 假玩家/白目玩家 額外遺物掉落（獨立判定）', keys: ['假玩家', '白目玩家', '遺物'], lines: [
      '擊殺假玩家/白目玩家 NPC 時，除了上面的裝備掉落，另有<b>獨立判定的 0.001% 機率</b>掉「遺物」品質道具（全部遺物池均等抽取）',
      '兩者互不排擠，可能同時觸發；裝備幸運暴走兔腳可讓這個機率變 <b>2 倍</b>（0.002%）',
      '攻城區域內擊殺不會觸發'
    ] },
    { id: 'holyrelic', title: '🏛️ 聖地遺物（拉斯塔巴德傳說武器材料）', keys: ['聖地遺物', '聖地的遺物', '死亡騎士之印記'], lines: [
      '<b>取得</b>：持有「死亡騎士之印記」時，在<b>拉斯塔巴德區域</b>擊敗<b>任何</b>怪物，<b>0.1%</b> 機率掉落（一般／經典同機率）。不在任何怪的固定掉落表內，所以直接搜怪查不到。',
