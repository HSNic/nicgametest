...
(function () {
  ...
  var ENHANCE_SECTIONS = [
    ...
    { t: '詞綴怎麼來', lines: [
      ...
      '<b>第5階(最高階)屬性武器可額外附加一個觸發技能</b>：用對應元素的『屬性強化卷軸』(火/水/風/地，怪物掉落)在碧恩身上施法，<b>1%</b>機率成功附加一個該元素的攻擊觸發魔法(失敗只消耗卷軸、武器不受影響)；<b>原生已內建攻擊/命中觸發技能的武器(含遺物)無法使用這個功能</b>。已附加過的也可以用同元素卷軸再次施法嘗試重抽換掉現有效果，機率一樣是1%，失敗原效果不變。' + '<span style="color:#64748b;font-size:11px;">(2026-07-21)</span>'
    ]}
  ];
  ...
  function renderPets() {
    ...
    var note = '<div class="m-wiki-note">「帶寵物」是<b>任何職業</b>都能玩的夥伴系統(v3.2.17全面改版，取代舊版吃肉/項圈/哨子玩法)：用專屬誘捕道具捕捉野外動物送進' + wN('包武') + '的' + wI('寵物保管') + '，寵物是有等級/經驗/HP/MP的<b>獨立實體</b>，最多同時<b>出戰4隻</b>幫你打怪。</div>' +
      '<div class="m-wiki-desc" style="color:#64748b;font-size:11px;">最後更新:2026-07-20(補袋鼠系穿甲說明)</div>';
    var lureRows = [
  ...
  function renderPvp() {
    ...
    out += '<div class="m-wiki-desc" style="color:#64748b;font-size:11px;">最後更新:' + TAB_UPDATED.pvp + '(v3.5.75~76新增性向值/PVP系統；v3.6.35新增白目玩家第二模板/汙濁之水異常、性向懲罰值加倍)</div>';
    return out;
  }
  ...
  function renderMode() {
    ...
    // 📅 最後更新:2026-07-17 —— 補經典模式新NPC「聖使阿卡塔」的死亡經驗買回機制
    var c6 = card('💀 死亡經驗買回（找聖使阿卡塔・僅經典模式）', '' +
      '<div class="m-wiki-desc">' + wN('聖使阿卡塔') + '(位於' + wL('亞丁') + '城左上迴廊前)<b>只有經典模式才會出現</b>——一般模式死亡不損失經驗，所以沒有這個NPC。</div>' +
      '<div class="m-wiki-desc" style="margin-top:4px;">・經典模式每次死亡損失經驗時，系統會記一筆「等級、損失量」的紀錄(<b>最多存10筆</b>，滿了自動淘汰最舊的)；不需要當下正在死亡狀態，隨時可以去找阿卡塔查看並買回。</div>' +
