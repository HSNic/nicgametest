...
const PLUGINS = [
  ...
  { file: 'afk-warehouse-skill.js', comment: '共用倉庫視覺註記:可學技能書顯示「可學」、角色不可使用的裝備/技能書顯示淡紅色外框，背包／倉庫兩側都標(純視覺;可獨立維護,原作者更新後重新加回此行即可)' },
  ...
  { file: 'afk-syslog-filter.js', comment: '系統與物品日誌加「系統/掉落/收購」訊息分類開關,比照戰鬥日誌既有的敵人/玩家等開關(可獨立維護,原作者更新後重新加回此行即可)' },
  ...
  { file: 'afk-track-shortcut.js', comment: '冒險地圖「黑市」按鈕旁新增「魔物追蹤」快捷鈕,等同主城奧貝勒/赫特/帝倫NPC功能(可獨立維護,原作者更新後重新加回此行即可)' },
  { file: 'afk-track-autorenew.js', comment: '魔物追蹤視窗內加「自動續約」開關,到期自動用同樣目標續約(可獨立維護,原作者更新後重新加回此行即可)' },
  { file: 'afk-pet-shortcut.js', comment: '冒險地圖快捷鈕列新增「寵物管理」,等同亞丁包武NPC功能(可獨立維護,原作者更新後重新加回此行即可)' },
  ...
  // afk-cloud-sync.js 已於 2026-07-09 暫停(使用者實測後認為體驗有問題),index.html 已移除引用，
  // 這裡也拿掉，避免下次自動同步又把 script 標籤補回去。檔案本體還留著，見
  // Lineage/加掛版/docs/交接與接手/雲端同步功能開發紀錄_暫停_20260709.md 的「重啟方式」。
];
